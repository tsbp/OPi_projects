...
#include <string.h>
...
#if (ST77xx_CS_Used)
  #define ST77xx_CS_HIGH()              digitalWrite(ST77xx_CS_Pin, 1) //GPIO_WriteBit(ST77xx_CS_Port, ST77xx_CS_Pin, Bit_SET) 
  #define ST77xx_CS_LOW()               digitalWrite(ST77xx_CS_Pin, 0) //GPIO_WriteBit(ST77xx_CS_Port, ST77xx_CS_Pin, Bit_RESET)
#else
  ...
  #define ST77xx_RESET_HIGH()           digitalWrite(ST77xx_RESET_Pin, 1) //GPIO_WriteBit(ST77xx_RESET_Port, ST77xx_RESET_Pin, Bit_SET) 
  #define ST77xx_RESET_LOW()            digitalWrite(ST77xx_RESET_Pin, 0) //GPIO_WriteBit(ST77xx_RESET_Port, ST77xx_RESET_Pin, Bit_RESET)
#endif

#if (ST77xx_DC_Used)
  #define ST77xx_DC_HIGH()              digitalWrite(ST77xx_DC_Pin, 1) //GPIO_WriteBit(ST77xx_DC_Port, ST77xx_DC_Pin, Bit_SET) 
  #define ST77xx_DC_LOW()               digitalWrite(ST77xx_DC_Pin, 0) //GPIO_WriteBit(ST77xx_DC_Port, ST77xx_DC_Pin, Bit_RESET)
#endif
...
void ST77xx_SendCmd(uint8_t Cmd)
{
  ...
  ST77xx_CS_LOW();
   ...
   wiringPiSPIDataRW (1, &Cmd, 1) ; //SPI_send8b(ST77xx_SPI_periph, &Cmd, 1);
...
void ST77xx_SendData(uint8_t Data)
{
  ...
  wiringPiSPIDataRW (1, &Data, 1) ;//SPI_send8b(ST77xx_SPI_periph, &Data, 1);
...
//==============================================================================
static void ST7789_WriteData(uint8_t *buff, int buff_size)
{
	//ST77xx_DC_HIGH();
  ST77xx_CS_LOW();
  ST77xx_DC_HIGH();
	...
	while (buff_size > 0) {
		...
		wiringPiSPIDataRW (1, buff, chunk_size); // HAL_SPI_Transmit(&ST7789_SPI_PORT, buff, chunk_size, HAL_MAX_DELAY);
		buff += chunk_size;
...
void ST77xx_FillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
  ...
  uint16_t* bg;
  bg  = malloc(h * w * sizeof(uint16_t));
  memset(bg, color, h * w * sizeof(uint16_t));
  ST7789_WriteData((uint8_t*)bg, h * w * sizeof(uint16_t));
  free(bg);
...
void ST77xx_DrawPartXY(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t *pBuff)
{
  ...
  for (uint16_t iy = y; iy < y + h; iy++)
  {
    ...
    // for (x = w; x > 0; x--)
    //   ST77xx_RamWrite(pBuff++, 1);
    ST7789_WriteData((uint8_t*)pBuff, w * sizeof(uint16_t));
  }
  //  ST77xx_SetWindow(x, y, x + w - 1, y + h - 1);
  //  ST7789_WriteData((uint8_t*)pBuff, h * w * sizeof(uint16_t));
}
//==============================================================================
...
void ST77xx_GPIO_init(void)
{
  ...
  pinMode (ST77xx_DC_Pin, OUTPUT) ;         // aka BCM_GPIO pin 17
  pinMode (ST77xx_RESET_Pin, OUTPUT) ;
  pinMode (ST77xx_CS_Pin, OUTPUT) ;

  int a = wiringPiSPISetup (1, 2000000) ;
